package com.example.config;

import com.example.tenant.TenantRoutingDataSource;
import com.zaxxer.hikari.HikariConfig;
import com.zaxxer.hikari.HikariDataSource;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
import org.springframework.context.annotation.Primary;

import javax.sql.DataSource;
import java.util.HashMap;

@Configuration
public class TenantDataSourceConfig {

    @Value("${spring.datasource.url}")
    private String jdbcUrl;

    @Value("${spring.datasource.username}")
    private String username;

    @Value("${spring.datasource.password}")
    private String password;

    @Bean("rawDataSource")
    public DataSource rawDataSource() {
        HikariConfig config = new HikariConfig();
        // Strip any currentSchema param — raw datasource uses server default
        config.setJdbcUrl(jdbcUrl.replaceAll("[?&]currentSchema=[^&]*", ""));
        config.setUsername(username);
        config.setPassword(password);
        config.setMaximumPoolSize(10);
        config.setMinimumIdle(2);
        config.setPoolName("HikariPool-default-customer");
        return new HikariDataSource(config);
    }

    @Bean
    @Primary
    public DataSource dataSource() {
        DataSource defaultDs = rawDataSource();

        // Base URL without any schema param — tenant URL built per-schema in TenantRoutingDataSource
        String baseUrl = jdbcUrl.replaceAll("[?&]currentSchema=[^&]*", "");

        TenantRoutingDataSource router = new TenantRoutingDataSource(
                baseUrl, username, password, "com.example.entity");
        router.setDefaultTargetDataSource(defaultDs);
        router.setTargetDataSources(new HashMap<>());
        router.afterPropertiesSet();
        return router;
    }
}
